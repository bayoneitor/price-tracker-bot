...
| All-time-low alerts | yes — a fifth `ThresholdType`, `core/alert.py` |
| Price history backfill | yes, plugin-based — `docs/history-providers.md`; ships no providers itself |
| Data | CSV import/export, per-user |
| Observability | Prometheus exporter, 14-panel Grafana dashboard |

---

*All-time-low alerts shipped* — see [docs/history-providers.md](history-providers.md) and the `all_time_low` entry in `core/alert.py`. What's below is what's left to build.

## 1. "Is this a good price?" — where today sits in the product's own history
...
## 3. Dedicated scrapers for Spanish retailers
...
**Not the same thing as history backfill.** The private Keepa/PrecioReal plugins (`docs/history-providers.md`) can already backfill *past* prices for some of these shops through PrecioReal's own aggregated data — this item is about reading a shop's *current* price directly, which is a different, narrower job (`AbstractScraper`, not `AbstractHistoryProvider`) and doesn't require PrecioReal to have the product at all.
...
already maintained. Small, and largely the same code path as the shipped
all-time-low trigger (`core/alert.py`).
...
## 9. Reference lines on the comparison chart

A single product's chart draws its target line; the group comparison marks
nothing. Each series' all-time low, and any group target from item 2, would make
...
- **Buy/sell recommendations or price forecasting.** The data supports describing
  the past, not predicting the future. Item 1 stays descriptive on purpose.
