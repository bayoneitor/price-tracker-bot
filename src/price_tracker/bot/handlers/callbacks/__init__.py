...
from price_tracker.bot.handlers.callbacks import _actions, _admin, _list, _menu, _ops, _product
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if await _ops.handle_ops_buttons(query, context, db, user_id, data):
        return
    if await _list.handle_list_navigation(query, context, db, user_id, data):
        return
